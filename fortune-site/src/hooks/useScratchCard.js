...
export function useScratchCard(card, { onProgress, onScratchActivity } = {}) {
  ...
  const textures = useMemo(() => {
    ...
    return { maskCanvas, maskTexture, coatingTexture, contentTexture, redrawContent };
  }, [card]);
  ...
  const scratchAt = useCallback((uv) => {
    ...
    if (!uv || !maskCanvasRef.current || !maskTexture || !contentTexture) return;
    ...
    applyDirectionalScratch(ctx, prev, point, velocity);
